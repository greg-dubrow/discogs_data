...
search_releases_lp <- function(country, year, format, key, secret, user_agent_string) {
  ...
  #  results <- json_data$results
    # Keep only what you need: id and optionally master_id
    releasedf <- dplyr::select(filtered, master_id, id, title,
      label, catno, format, genre, style, community.want, community.have)
    releasedf <- releasedf %>%
      dplyr::filter(!purrr::map_lgl(format, ~ "EP" %in% .x))
    releasedf <- releasedf %>%
      dplyr::filter(!purrr::map_lgl(format, ~ "Test Pressing" %in% .x))
    releasedf <- releasedf %>%
      dplyr::filter(!purrr::map_lgl(format, ~ "Promo" %in% .x))
    all_data[[page]] <- tibble::as_tibble(releasedf)
...
search_releases_ep <- function(country, year, format, key, secret, user_agent_string) {
  # Ensure format is correctly handled
  valid_formats <- c("Vinyl", "CD", "Cassette")
  if (!format %in% valid_formats) {
    stop("Unsupported format. Please choose one of: ", paste(valid_formats, collapse = ", "))
  ...
  repeat {
    ...
    releasedf <- dplyr::select(filtered, master_id, id, title,
      label, catno, format, genre, style, community.want, community.have)
    # Remove any rows containing “LP” in the format list
    releasedf <- releasedf %>%
      dplyr::filter(!purrr::map_lgl(format, ~ "LP" %in% .x))
    releasedf <- releasedf %>%
      dplyr::filter(!purrr::map_lgl(format, ~ "Test Pressing" %in% .x))
    releasedf <- releasedf %>%
      dplyr::filter(!purrr::map_lgl(format, ~ "Promo" %in% .x))
...
# search releases_single() -------------------------------------------------------

search_releases_single <- function(country, year, format, key, secret, user_agent_string) {
  # Ensure format is correctly handled
  valid_formats <- c("Vinyl", "CD", "Cassette")
  if (!format %in% valid_formats) {
    stop("Unsupported format. Please choose one of: ", paste(valid_formats, collapse = ", "))
  ...
  repeat {
    ...
    # Keep only what you need: id and optionally master_id
    # releasedf <- dplyr::select(filtered, master_id, id, title,
    #   label, catno, format, genre, style)
    releasedf <- dplyr::select(filtered, master_id, id, title,
      label, catno, format, genre, style, community.want, community.have)
    # Remove any rows containing “LP” in the format list
    releasedf <- releasedf %>%
...
# --- Helper function to expand a list column into separate columns ---
expand_list_column <- function(data, colname, prefix = colname, max_items = NULL) {
  col <- sym(colname)

  # Limit list length per row if requested
  trimmed <- data[[colname]] %>%
    purrr::map(~ {
      if (is.null(.x)) return(NA_character_)
      if (!is.null(max_items)) .x <- .x[seq_len(min(length(.x), max_items))]
      as.character(.x)
    })

  # Replace original column
  data[[colname]] <- trimmed

  # Determine maximum actual length
  max_len <- max(purrr::map_int(data[[colname]], length), na.rm = TRUE)
  new_names <- paste0(prefix, "_", seq_len(max_len))

  # Expand the list column into separate columns
  data %>%
    tidyr::unnest_wider(!!col, names_sep = "_", names_repair = "unique") %>%
    rename_with(~ new_names[seq_along(.)], starts_with(paste0(colname, "_")))
}

expand_list_column <- function(data, colname, prefix = colname) {
  # Convert symbol
  col <- rlang::sym(colname)

  # Find max length across rows
  max_len <- max(purrr::map_int(data[[colname]], length), na.rm = TRUE)
